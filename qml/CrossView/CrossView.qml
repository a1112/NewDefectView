...
property var t : [{"seqNo":616748,"imageCount":15,"steelLen":
    ...
    onY_borderChanged: {
            let t_Value=imageViewCore.imageWidthInfoList[showInageIndex_x]["steelLen"][showInageIndex_y]
            let b_Value=imageViewCore.imageWidthInfoList[showInageIndex_x]["steelLen"][showInageIndex_y+1]
            mm_h=t_Value+(b_Value-t_Value)*(root.cross_y/imageShowHeight-showInageIndex_y)
    }
    property int x_border: {
        if (imageCore.isMaxImageModel){
            return imageCore.hoverdListIndex*coreConfig.imageWidth+root.cross_x/imageShowWidth*coreConfig.imageWidth
        }
        else{
            return root.cross_x/imageShowWidth*coreConfig.imageWidth
        }

    }

    onX_borderChanged: {
        let l_Value=imageViewCore.imageWidthInfoList[showInageIndex_x]["steelLeft"][showInageIndex_y]
        let r_Value=imageViewCore.imageWidthInfoList[showInageIndex_x]["steelRight"][showInageIndex_y]
